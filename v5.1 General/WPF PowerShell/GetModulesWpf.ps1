...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <!-- Grid is the root layout container - provides structured rows/columns -->
    <Grid>
        <!-- RowDefinitions define the vertical layout structure -->
        <Grid.RowDefinitions>
            <RowDefinition Height="Auto"/>    <!-- Toolbar: Auto-sized to content -->
            <RowDefinition Height="*"/>       <!-- DataGrid: Takes remaining space -->
            <RowDefinition Height="Auto"/>    <!-- StatusBar: Auto-sized to content -->
        </Grid.RowDefinitions>
        
        <!-- Toolbar Section: Grid.Row="0" -->
        <!-- StackPanel: Arranges children horizontally -->
        <StackPanel Grid.Row="0" Orientation="Horizontal" Margin="10,10,10,5" Background="LightGray">
            <!-- Named controls for event handler attachment -->
            ...
            <Separator Width="10"/>  <!-- Visual spacer element -->
            <!-- TextBox with vertical content alignment for better UX -->
            <TextBox Name="SearchBox" Width="200" Height="25" Margin="5" 
                     VerticalContentAlignment="Center"/>
            <Label Content="Search:" VerticalAlignment="Center" Margin="0,0,5,0"/>
            <Separator Width="10"/>  <!-- Another visual spacer -->
            <!-- CheckBox for filtering functionality -->
            <CheckBox Name="ShowOnlyLoadedCheckBox" Content="Show Only Loaded" VerticalAlignment="Center" Margin="5"/>
        </StackPanel> <!-- End StackPanel (Toolbar) -->
        
        <!-- Data Grid Section: Grid.Row="1" -->
        <!-- DataGrid: Advanced data presentation control with sorting, selection, etc. -->
        <DataGrid Grid.Row="1" Name="ModulesDataGrid" Margin="10,5,10,5"
                  AutoGenerateColumns="False"    <!-- Manual column definition for control -->
                  CanUserAddRows="False"         <!-- Read-only data presentation -->
                  CanUserDeleteRows="False"      <!-- Prevent data modification -->
                  IsReadOnly="True"              <!-- No in-place editing -->
                  GridLinesVisibility="All"      <!-- Visual grid lines -->
                  HeadersVisibility="Column"     <!-- Show column headers only -->
                  AlternatingRowBackground="LightBlue"  <!-- Zebra striping for readability -->
                  SelectionMode="Single">        <!-- Single row selection -->
            <!-- Manual column definitions with data binding -->
            <DataGrid.Columns>
                <!-- Each column uses {Binding PropertyName} syntax for data binding -->
                ...
                <!-- Width="*" means take remaining available space -->
                <DataGridTextColumn Header="Path" Binding="{Binding Path}" Width="*"/>
            </DataGrid.Columns> <!-- End DataGrid.Columns -->
        </DataGrid> <!-- End DataGrid -->
        
        <!-- Status Bar Section: Grid.Row="2" -->
        <StatusBar Grid.Row="2" Height="25">
            <!-- Label for displaying application status information -->
            <Label Name="StatusLabel" Content="Ready"/>
        </StatusBar> <!-- End StatusBar -->
    </Grid> <!-- End Grid (Root layout container) -->
</Window> <!-- End Window (Root WPF element) -->
"@
