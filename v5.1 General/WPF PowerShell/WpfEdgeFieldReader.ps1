    ...
    try {
        ...
        for ($i = 0; $i -lt $items.Count; $i++) {
            try {
                ...
                Write-Log "Processing element $i - Type: $controlType, Name: '$name'"
                ...
                Write-Log "  Added field: '$label' (Value length: $($value.Length))"
            } catch {
        ...
        <Border Grid.Row="2" Background="White" BorderBrush="#FFB0C4DE" BorderThickness="1" CornerRadius="6" Padding="10">
            <StackPanel>
                ...
                <WrapPanel Margin="0,0,0,8">
                    <CheckBox x:Name="GraphicsCheckBox" Content="Graphics" IsChecked="False"/>
                    <CheckBox x:Name="TextAreasCheckBox" Content="Text Areas" IsChecked="False"/>
...
function Update-WindowsTabsList {
    ...
    try {
        ...
        $displayItems = @()
        foreach ($window in $script:EdgeWindows) {
            foreach ($tab in $window.Tabs) {
                $displayItems += [PSCustomObject]@{
                    DisplayName = "$($window.WindowTitle) - $($tab.Name)"
                    Window = $window
                    Tab = $tab
                    Element = $tab.Element
                }
...
function Update-FieldsList {
    ...
    $filtered = $script:EdgeFieldEntries | Where-Object {
        ...
        (($tag -notin @('image','textarea')) -and $otherTypesCheckBox -and $otherTypesCheckBox.IsChecked)
    }
